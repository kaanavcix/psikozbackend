...
export class AppointmentController {
  ...
  appointmentControl: any = async (req: Request, res: Response) => {
    ...
    const { user_username, is_received, doctor_username, date, hours, is_appointment }: AppointmentModel = req.body;
    ...
    if (appointment.length !== 0) {
      ...
      return res.status(201).send({
        status: false,
        message: isWhich.is_appointment ? "Bu randevuya dair istek oluşturulmuştur" : (isWhich.is_received ? "Bu randevu daha önce alınmıştır" : "Randevu şuanda müsait durumdadır")
      })
